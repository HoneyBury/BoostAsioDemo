# Boost 游戏服务器框架 v1.0.0（develop 维护期）

基于 Boost.Asio 构建的高性能 C++20 游戏服务器框架。

> **版本基线说明**
>
> - `v1.0.0` 发布版对应 git tag `v1.0.0`（commit `22defe8`），是稳定承诺的最小发布面。
> - 当前 `develop` 分支已进入 **`v1.x` 维护期**，会按 `docs/development-optimization.md` §11 的版本批次（`v1.1.1` → `v1.1.17` → `v1.2.x`）持续做"事实源校准 / 主链收口 / 边界测试加固"。
> - **本 README 中的能力描述与 `docs/v1-maturity-matrix.md` 冲突时以矩阵为准**。矩阵是维护期的单一事实源。
> - `v2.0.0`（Actor + ECS 混合架构）在 `v1.2.0` 决策点之前不进入开发，详见 `docs/v2-roadmap.md` / `docs/v2-design.md`。
...
- **测试体系（stable）**：当前 `ctest --preset windows-msvc-debug -N` 枚举共 **54 个用例**（14 个单元测试源文件 + 2 个集成测试源文件 + `packet_fuzz_test.cpp` 模糊测试，GoogleTest `gtest_discover_tests` 展开后）
