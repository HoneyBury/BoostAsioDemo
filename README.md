...
> - 当前发布版为 **`v3.3.0`**（2026-05-14），全部模块集成到生产链，**780 测试通过**。
> - v3.3.0 整体评级为"企业"阶段，P0-P3 全部 13 个模块已接入生产链路。
...
v3.3.0 在 v2.0.0 基础上完成了全部 13 个模块的生产链路接入。
核心架构覆盖 Actor 模型、多核 I/O、内存管理、服务拆分、数据持久化、ECS 战斗世界、运维成熟度、分布式运行时、生产基础设施（Redis/Docker/K8s/TLS）。
...
├── v3/                     # v3.0.0 分布式
│   ├── cluster/            ClusterRouter(P1a), Raft, ConsistentHash, SecurityPolicy
