...
说明：普通 branch push / PR 不再自动触发流水线；自动触发只保留特定 release tag，当前约定为 `v*`。`.github/runner-matrix.json` 作为版本化 runner/默认标签配置源。本文件讨论的是手动 fixed-runner 证据流水线口径，不替代真实 workflow 配置。
