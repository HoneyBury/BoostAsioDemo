...
static RaftConfig make_config(const std::string& node_id,
                              const std::vector<RaftNodeId>& peers) {
    RaftConfig cfg;
    cfg.node_id = node_id;
    cfg.peers = peers;
...
TEST_F(RaftE2ETest, E2E_OnlyOneLeader) {
    ...
    for (const auto& [_, n] : nodes_) {
        if (n.node && n.node->is_leader()) ++count;
    }
    EXPECT_EQ(count, 1);
}
