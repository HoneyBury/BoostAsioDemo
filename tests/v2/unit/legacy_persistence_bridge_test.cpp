...
TEST(LegacyPersistenceBridgeTest, JsonFilePlayerStoreRoundTrip) {
    const auto dir = std::filesystem::temp_directory_path() / "v2_player_store_round_trip";
...
#ifdef HAS_SQLITE
TEST(LegacyPersistenceBridgeTest, SqlitePlayerStoreRoundTrip) {
    const auto path = std::filesystem::temp_directory_path() / "v2_player_store_round_trip.sqlite";
...
TEST(LegacyPersistenceBridgeTest, JsonFileBattleReplayStoreRoundTrip) {
    const auto dir = std::filesystem::temp_directory_path() / "v2_battle_replay_round_trip";
    std::filesystem::remove_all(dir);
...
TEST(LegacyPersistenceBridgeTest, ReplayPlayerLoadsFramesAndPlaysToCompletion) {
    const auto dir = std::filesystem::temp_directory_path() / "v2_replay_player_round_trip";
    std::filesystem::remove_all(dir);
...
TEST(LegacyPersistenceBridgeTest, AuditLogAppendsApproxJsonLine) {
    app::logging::init("project_v2_unit_tests");
